...
/**
 * @brief Callback for the flash timer.
 * @details Forces a view update to revert highlighted keys to their normal state.
 * @param context The application context (PageFlipperApp*).
 */
...
/**
 * @brief Worker thread for polling GPIO pins.
 * @details Detects single and double presses on PA6 and PA7 foot pedals.
 * @param context The application context (PageFlipperApp*).
...
/**
 * @brief Draw callback for the Help screen.
 * @details Renders help text, page indicators, and navigation icons.
 * @param canvas The canvas to draw on.
 * @param model The HelpModel pointer.
 */
...
/**
 * @brief Input callback for the Help screen.
 * @details Handles page navigation and returning to the main view.
 * @param event The input event (key press).
...
/**
 * @brief Draw callback for the Main screen.
 * @details Renders arrow key status, connection status, and info line.
 * @param canvas The canvas to draw on.
 * @param model The PageFlipperModel pointer.
 */
static void page_flipper_draw_callback(Canvas* canvas, void* model) {
    ...
    canvas_draw_line(canvas, 64, 19, 68, 22);
...
/**
 * @brief Sends a keyboard HID key via BLE.
 * @details Handles the HID key press/release sequence and starts the flash timer.
 * @param app The application context.
 * @param hid_key The HID key code to send.
 */
...
/**
 * @brief Callback for Bluetooth status changes.
 * @details Updates the connection state in the UI model.
 * @param status The new Bluetooth status.
 * @param context The application context.
 */
 ...
 * @details Handles arrow key presses, OK (Help), and Back (Exit).
...
/**
 * @brief Custom event callback for the View Dispatcher.
 * @details Handles events generated by the GPIO polling thread.
 * @param context The application context.
 * @param event The custom event ID.
 * @return true if the event was handled.
 */
...
/**
 * @brief Allocates and initializes the PageFlipper application.
 * @details Initializes GUI, Bluetooth, Views, GPIOs, and the worker thread.
 * @return Pointer to the allocated PageFlipperApp.
 */
...
/**
 * @brief Deinitializes and frees the PageFlipper application.
 * @details Stops the worker thread, releases GPIOs, and restores Bluetooth settings.
 * @param app The PageFlipperApp instance.
 */
